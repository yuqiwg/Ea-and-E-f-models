...
for dirname in [dirs for dirs in os.walk(path)][0][1]:
    ...
    for name in descriptors:
        for spe in species_type:
            locals()['f_X_train_%s_%s' % (name,spe)]=np.load('../../data/E_and_f/f_%s_trainx_%s.npy' % (name,spe))
            locals()['f_y_train_%s_%s' % (name,spe)]=np.load('../../data/E_and_f/f_%s_trainy_%s.npy' % (name,spe))
